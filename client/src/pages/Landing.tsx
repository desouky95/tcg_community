  ...
  return (
    <div className="wax-page">
      <header className="wax-header">
        <Link to="/" className="wax-brand" aria-label="TCG Nexus home">
          <span className="wax-brand-mark">TN</span>
          <span><strong>TCG NEXUS</strong><small>COLLECT · TRADE · BELONG</small></span>
        </Link>

        <nav className={`wax-nav ${menuOpen ? "is-open" : ""}`} aria-label="Primary navigation">
          <Link to="/checklists">How it works</Link>
          <Link to="/checklists">The sets</Link>
          <Link to="/marketplace">Marketplace</Link>
          <Link to="/swapping">Swaps</Link>
          <Link to="/login" className="wax-nav-mobile-action">Sign in</Link>
          <Link to="/signup" className="wax-button wax-button-small wax-nav-mobile-action">Join the club</Link>
        </nav>

        <div className="wax-header-actions">
          <Link to="/login" className="wax-signin">Sign in</Link>
          <button type="button" className="wax-menu-button" aria-expanded={menuOpen} aria-label={menuOpen ? "Close menu" : "Open menu"} onClick={() => setMenuOpen((open) => !open)}>
      ...
      <footer className="wax-footer"><span className="wax-brand-footer">TCG NEXUS</span><span>Cards bring people closer.</span><span>© 2026 · Egypt first, collectors everywhere.</span></footer>
    </div>
  );
}
