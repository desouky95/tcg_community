...
export default function Chat() {
  ...
  return (
    <Layout>
      <div className="bg-card border border-border shadow-md rounded-2xl overflow-hidden h-[calc(100vh-8rem)] min-h-[600px] flex rtl:flex-row-reverse">
                ...
                {conversations?.map((conv: ConversationListDto) => (
                  <button
                    ...
                    className={`w-full p-4 flex items-start gap-4 transition-all rtl:text-right
                      ${Number(id || -1) === conv.id ? "bg-primary-500/10 border-l-4 border-primary-500 rtl:border-l-0 rtl:border-r-4" : "hover:bg-input/20 border-l-4 border-transparent rtl:border-r-4"}`}
                  >
