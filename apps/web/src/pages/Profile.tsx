...
export default function Profile() {
  ...
  const targetId = id || (currentUser?.id ? String(currentUser.id) : undefined);
  ...
  const handleMessageClick = async () => {
    ...
    try {
      const conv = await createConversation.mutateAsync(Number(profileUser.id));
  ...
  return (
    <Layout>
      <div className="wax-workspace-view wax-profile-view grid grid-cols-1 lg:grid-cols-3 gap-8">
