...
| Surface | `seller inventory` |
| Dependencies | [LIST-010](../marketplace/list-010-publish-listing.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **pause listing**. Success means: The seller removes a published listing from discovery without deleting history. The scenario starts when an eligible seller initiates the action from `seller inventory` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/listings/:id/pause`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **pause listing** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across listings.
...
6. The client renders the canonical result and the next valid action. The seller removes a published listing from discovery without deleting history.
...
- Emit `marketplace.pause-listing.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/listings/:id/pause`.
...
`mutation pauseListing` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The seller removes a published listing from discovery without deleting history.
...
- Integration: `POST /api/v1/listings/:id/pause`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
