...
| Dependencies | [LIST-007](../marketplace/list-007-create-listing-draft.md), [LIST-008](../marketplace/list-008-upload-listing-media.md), [LIST-009](../marketplace/list-009-set-listing-price-quantity.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **publish listing**. Success means: A complete draft enters published or moderation-pending state. The scenario starts when an eligible seller initiates the action from `/marketplace/sell` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/listings/:id/publish`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **publish listing** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. A complete draft enters published or moderation-pending state.
...
- Emit `marketplace.publish-listing.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/listings/:id/publish`.
...
`mutation publishListing` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A complete draft enters published or moderation-pending state.
...
- Integration: `POST /api/v1/listings/:id/publish`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
