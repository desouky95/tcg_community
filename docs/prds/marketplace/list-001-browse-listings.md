# LIST-001: Browse published listings
...
**Current evidence:** apps/web/src/pages/Marketplace.tsx uses mock data.

## Problem and outcome

The product needs a consistent, authorized way to **browse published listings**. Success means: Published listings are paginated in stable order. The scenario starts when an eligible guest initiates the action from `/marketplace` and ends when the canonical result is persisted or a typed terminal error is returned.
...
3. Marketplace module validates the business invariant for **browse published listings** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from listings, listing_media.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Published listings are paginated in stable order.
...
- Emit `marketplace.browse-listings.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`query browseListings` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Published listings are paginated in stable order.
