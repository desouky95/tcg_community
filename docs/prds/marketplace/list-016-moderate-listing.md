...
| Dependencies | [LIST-010](../marketplace/list-010-publish-listing.md), [LIST-015](../marketplace/list-015-report-listing.md) |

**Current evidence:** Planned admin route.

## Problem and outcome

The product needs a consistent, authorized way to **moderate listing**. Success means: A moderator approves, rejects, hides, or restores a listing with reasons. The scenario starts when an eligible moderator initiates the action from `/admin/listings` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/admin/listings/:id/decision`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **moderate listing** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across listings, listing_reports, audit_logs.
...
6. The client renders the canonical result and the next valid action. A moderator approves, rejects, hides, or restores a listing with reasons.
...
- Emit `marketplace.moderate-listing.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/admin/listings/:id/decision`.
...
`mutation moderateListing` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A moderator approves, rejects, hides, or restores a listing with reasons.
...
- Integration: `POST /api/v1/admin/listings/:id/decision`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
