...
2. The interface invokes `POST /api/v1/listings/:id/archive`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **archive listing** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. A listing without active reservations or orders becomes archived.
...
- Emit `marketplace.archive-listing.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/listings/:id/archive`.
...
`mutation archiveListing` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A listing without active reservations or orders becomes archived.
...
- Integration: `POST /api/v1/listings/:id/archive`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
