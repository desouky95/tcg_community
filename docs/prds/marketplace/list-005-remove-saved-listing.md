...
| Dependencies | [LIST-004](../marketplace/list-004-save-listing.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **remove saved listing**. Success means: The saved reference is removed idempotently. The scenario starts when an eligible buyer initiates the action from `saved listings` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `DELETE /api/v1/listings/:id/save`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **remove saved listing** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across saved_listings.
...
6. The client renders the canonical result and the next valid action. The saved reference is removed idempotently.
...
- Emit `marketplace.remove-saved-listing.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `DELETE /api/v1/listings/:id/save`.
...
`mutation removeSavedListing` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The saved reference is removed idempotently.
...
- Integration: `DELETE /api/v1/listings/:id/save`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
