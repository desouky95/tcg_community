...
| Dependencies | [LIST-012](../marketplace/list-012-pause-listing.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **resume listing**. Success means: An eligible paused listing returns to published or moderation-pending state. The scenario starts when an eligible seller initiates the action from `seller inventory` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/listings/:id/resume`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **resume listing** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. An eligible paused listing returns to published or moderation-pending state.
...
- Emit `marketplace.resume-listing.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/listings/:id/resume`.
...
`mutation resumeListing` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: An eligible paused listing returns to published or moderation-pending state.
...
- Integration: `POST /api/v1/listings/:id/resume`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
