# LIST-002: Search filter and sort listings
...
**Current evidence:** Marketplace UI filters mock data.

## Problem and outcome

The product needs a consistent, authorized way to **search filter and sort listings**. Success means: Normalized query, facets, and sort return stable cursors. The scenario starts when an eligible guest initiates the action from `/marketplace` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/listings`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **search filter and sort listings** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from listings.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Normalized query, facets, and sort return stable cursors.
...
- Emit `marketplace.search-filter-sort-listings.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/listings`.
...
`query searchFilterSortListings` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Normalized query, facets, and sort return stable cursors.
...
- Integration: `GET /api/v1/listings`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
