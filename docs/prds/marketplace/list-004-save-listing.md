...
The product needs a consistent, authorized way to **save listing**. Success means: The listing is idempotently saved for the buyer. The scenario starts when an eligible buyer initiates the action from `/marketplace/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/listings/:id/save`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **save listing** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. The listing is idempotently saved for the buyer.
...
- Emit `marketplace.save-listing.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/listings/:id/save`.
...
`mutation saveListing` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The listing is idempotently saved for the buyer.
...
- Integration: `POST /api/v1/listings/:id/save`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
