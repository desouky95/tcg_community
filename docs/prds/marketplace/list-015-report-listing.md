...
The product needs a consistent, authorized way to **report listing**. Success means: A deduplicated report enters moderation. The scenario starts when an eligible buyer initiates the action from `/marketplace/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/listings/:id/report`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **report listing** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across listing_reports.
...
6. The client renders the canonical result and the next valid action. A deduplicated report enters moderation.
...
- Emit `marketplace.report-listing.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/listings/:id/report`.
...
`mutation reportListing` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A deduplicated report enters moderation.
...
- Integration: `POST /api/v1/listings/:id/report`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
