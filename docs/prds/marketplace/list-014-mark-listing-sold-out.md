# LIST-014: Mark listing sold out
...
The product needs a consistent, authorized way to **mark listing sold out**. Success means: Zero available quantity transitions the listing to sold_out once. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `ListingInventoryPolicy`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **mark listing sold out** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across listings, orders.
...
6. The worker records the canonical result; dependent clients observe it through their owning query or event. Zero available quantity transitions the listing to sold_out once.
...
- Emit `marketplace.mark-listing-sold-out.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `ListingInventoryPolicy`.
...
- Given valid prerequisites and authorization, the scenario ends with: Zero available quantity transitions the listing to sold_out once.
...
- Integration: `ListingInventoryPolicy`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
