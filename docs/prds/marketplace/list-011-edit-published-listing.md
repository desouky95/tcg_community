# LIST-011: Edit published listing
...
2. The interface invokes `PATCH /api/v1/listings/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **edit published listing** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Allowed changes create a new version and may require remoderation.
...
- Emit `marketplace.edit-published-listing.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PATCH /api/v1/listings/:id`.
...
`mutation editPublishedListing` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Allowed changes create a new version and may require remoderation.
...
- Integration: `PATCH /api/v1/listings/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
