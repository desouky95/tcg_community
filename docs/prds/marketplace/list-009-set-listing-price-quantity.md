# LIST-009: Set listing price and quantity
...
The product needs a consistent, authorized way to **set listing price and quantity**. Success means: Minor-unit price, currency, and available quantity are validated. The scenario starts when an eligible seller initiates the action from `/marketplace/sell` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PATCH /api/v1/listings/:id/pricing`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **set listing price and quantity** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Minor-unit price, currency, and available quantity are validated.
...
- Emit `marketplace.set-listing-price-quantity.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PATCH /api/v1/listings/:id/pricing`.
...
`mutation setListingPriceQuantity` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Minor-unit price, currency, and available quantity are validated.
...
- Integration: `PATCH /api/v1/listings/:id/pricing`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
