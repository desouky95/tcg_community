# LIST-006: Contact listing seller
...
| Dependencies | [LIST-003](../marketplace/list-003-view-listing-detail.md), [CHAT-002](../chat/chat-002-find-create-conversation.md) |

**Current evidence:** Mock CTA only.

## Problem and outcome

The product needs a consistent, authorized way to **contact listing seller**. Success means: An eligible buyer enters a listing-linked conversation. The scenario starts when an eligible buyer initiates the action from `/marketplace/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/listings/:id/contact`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Marketplace module validates the business invariant for **contact listing seller** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. An eligible buyer enters a listing-linked conversation.
...
- Emit `marketplace.contact-listing-seller.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/listings/:id/contact`.
...
`mutation contactListingSeller` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: An eligible buyer enters a listing-linked conversation.
...
- Integration: `POST /api/v1/listings/:id/contact`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
