...
| Surface | `/swapping` |
| Dependencies | [COL-006](../collections/col-006-track-duplicates-offers.md), [COL-007](../collections/col-007-track-wanted-missing.md) |

**Current evidence:** apps/api SwapsController.search.

## Problem and outcome

The product needs a consistent, authorized way to **search swap partners**. Success means: Collectors matching card and location criteria are ranked. The scenario starts when an eligible collector initiates the action from `/swapping` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/swaps/search`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Swap module validates the business invariant for **search swap partners** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from users, collection_entries.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Collectors matching card and location criteria are ranked.
...
- Emit `swaps.search-swap-partners.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/swaps/search`.
...
`query searchSwapPartners` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Collectors matching card and location criteria are ranked.
...
- Integration: `POST /api/v1/swaps/search`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
