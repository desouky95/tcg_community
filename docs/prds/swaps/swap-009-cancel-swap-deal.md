...
The product needs a consistent, authorized way to **cancel swap deal**. Success means: An eligible participant cancels before completion. The scenario starts when an eligible collector initiates the action from `/swaps/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/swap-deals/:id/cancel`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Swap module validates the business invariant for **cancel swap deal** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. An eligible participant cancels before completion.
...
- Emit `swaps.cancel-swap-deal.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/swap-deals/:id/cancel`.
...
`mutation cancelSwapDeal` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: An eligible participant cancels before completion.
...
- Integration: `POST /api/v1/swap-deals/:id/cancel`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
