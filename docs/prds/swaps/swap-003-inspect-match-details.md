# SWAP-003: Inspect match details
...
| Surface | `/swapping` |
| Dependencies | [SWAP-002](../swaps/swap-002-calculate-bilateral-match.md) |

**Current evidence:** apps/web/src/pages/Swapping.tsx.

## Problem and outcome

The product needs a consistent, authorized way to **inspect match details**. Success means: A collector can verify quantities and counterpart reputation. The scenario starts when an eligible collector initiates the action from `/swapping` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/swaps/:userId`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Swap module validates the business invariant for **inspect match details** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from collection_entries, users.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. A collector can verify quantities and counterpart reputation.
...
- Emit `swaps.inspect-match-details.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/swaps/:userId`.
...
`query inspectMatchDetails` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A collector can verify quantities and counterpart reputation.
...
- Integration: `GET /api/v1/swaps/:userId`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
