# SWAP-013: Record postal tracking
...
| Surface | `/chat/:id` |
| Dependencies | [SWAP-012](../swaps/swap-012-choose-postal-exchange.md) |

**Current evidence:** Postal endpoint exists.

## Problem and outcome

The product needs a consistent, authorized way to **record postal tracking**. Success means: A participant records a normalized optional tracking reference. The scenario starts when an eligible collector initiates the action from `/chat/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/swap-deals/:id/postal`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Swap module validates the business invariant for **record postal tracking** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. A participant records a normalized optional tracking reference.
...
- Emit `swaps.record-swap-tracking.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/swap-deals/:id/postal`.
...
`mutation recordSwapTracking` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A participant records a normalized optional tracking reference.
...
- Integration: `POST /api/v1/swap-deals/:id/postal`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
