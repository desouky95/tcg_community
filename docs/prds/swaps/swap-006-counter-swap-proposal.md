# SWAP-006: Counter swap proposal
...
**Current evidence:** No current counter endpoint.

## Problem and outcome

The product needs a consistent, authorized way to **counter swap proposal**. Success means: A recipient-authored proposal version supersedes the pending version. The scenario starts when an eligible collector initiates the action from `/swaps/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/swap-deals/:id/counter`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Swap module validates the business invariant for **counter swap proposal** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. A recipient-authored proposal version supersedes the pending version.
...
- Emit `swaps.counter-swap-proposal.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/swap-deals/:id/counter`.
...
`mutation counterSwapProposal` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A recipient-authored proposal version supersedes the pending version.
...
- Integration: `POST /api/v1/swap-deals/:id/counter`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
