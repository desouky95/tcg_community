# SWAP-012: Choose postal exchange
...
**Current evidence:** apps/api SwapDealsController.updatePostal.

## Problem and outcome

The product needs a consistent, authorized way to **choose postal exchange**. Success means: Postal details are shared only with participants. The scenario starts when an eligible collector initiates the action from `/chat/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
3. Swap module validates the business invariant for **choose postal exchange** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Postal details are shared only with participants.
...
- Emit `swaps.choose-postal-exchange.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation choosePostalExchange` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Postal details are shared only with participants.
