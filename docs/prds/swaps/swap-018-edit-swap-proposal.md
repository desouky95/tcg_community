# SWAP-018: Edit pending swap proposal
...
**Current evidence:** No current edit endpoint.

## Problem and outcome

The product needs a consistent, authorized way to **edit pending swap proposal**. Success means: The original proposer replaces items on a still-pending proposal using optimistic concurrency. The scenario starts when an eligible collector initiates the action from `/swaps/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PATCH /api/v1/swap-deals/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Swap module validates the business invariant for **edit pending swap proposal** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across swap_deals, swap_deal_items.
...
6. The client renders the canonical result and the next valid action. The original proposer replaces items on a still-pending proposal using optimistic concurrency.
...
- Emit `swaps.edit-swap-proposal.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PATCH /api/v1/swap-deals/:id`.
...
`mutation editSwapProposal` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The original proposer replaces items on a still-pending proposal using optimistic concurrency.
...
- Integration: `PATCH /api/v1/swap-deals/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
