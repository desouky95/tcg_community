...
| Surface | `/chat/:id` |
| Dependencies | [SWAP-004](../swaps/swap-004-start-conversation-from-match.md) |

**Current evidence:** apps/api SwapDealsController.store.

## Problem and outcome

The product needs a consistent, authorized way to **propose swap deal**. Success means: A pending proposal snapshots selected items. The scenario starts when an eligible collector initiates the action from `/chat/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/swap-deals`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Swap module validates the business invariant for **propose swap deal** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. A pending proposal snapshots selected items.
...
- Emit `swaps.propose-swap-deal.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/swap-deals`.
...
`mutation proposeSwapDeal` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A pending proposal snapshots selected items.
...
- Integration: `POST /api/v1/swap-deals`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
