# SWAP-010: Expire stale swap proposal
...
**Current evidence:** No current expiry worker.

## Problem and outcome

The product needs a consistent, authorized way to **expire stale swap proposal**. Success means: Pending proposals past expires_at become expired once. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `SwapExpiryJob`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Swap module validates the business invariant for **expire stale swap proposal** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Pending proposals past expires_at become expired once.
...
- Emit `swaps.expire-swap-proposal.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `SwapExpiryJob`.
...
`mutation expireSwapProposal` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Pending proposals past expires_at become expired once.
...
- Integration: `SwapExpiryJob`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
