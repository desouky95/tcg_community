...
**Current evidence:** No explicit client message id contract.

## Problem and outcome

The product needs a consistent, authorized way to **retry failed message**. Success means: The same client_message_id cannot create duplicates. The scenario starts when an eligible collector initiates the action from `/chat/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
3. Conversation module validates the business invariant for **retry failed message** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. The same client_message_id cannot create duplicates.
...
- Emit `chat.retry-failed-message.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation retryFailedMessage` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The same client_message_id cannot create duplicates.
