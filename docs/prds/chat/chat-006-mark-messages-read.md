...
The product needs a consistent, authorized way to **mark messages read**. Success means: The participant read cursor advances monotonically. The scenario starts when an eligible collector initiates the action from `/chat/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/conversations/:id/read`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Conversation module validates the business invariant for **mark messages read** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across conversation_participants.
...
6. The client renders the canonical result and the next valid action. The participant read cursor advances monotonically.
...
- Emit `chat.mark-messages-read.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/conversations/:id/read`.
...
`mutation markMessagesRead` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The participant read cursor advances monotonically.
...
- Integration: `POST /api/v1/conversations/:id/read`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
