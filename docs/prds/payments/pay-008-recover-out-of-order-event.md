# PAY-008: Recover out-of-order payment event
...
The product needs a consistent, authorized way to **recover out-of-order payment event**. Success means: Events are retained and projected only through legal state transitions. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PaymentStateProjectionJob`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Payment module validates the business invariant for **recover out-of-order payment event** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across payment_events, payment_intents.
...
6. The worker records the canonical result; dependent clients observe it through their owning query or event. Events are retained and projected only through legal state transitions.
...
- Emit `payments.recover-out-of-order-event.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PaymentStateProjectionJob`.
...
- Given valid prerequisites and authorization, the scenario ends with: Events are retained and projected only through legal state transitions.
...
- Integration: `PaymentStateProjectionJob`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
