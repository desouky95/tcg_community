# PAY-014: Generate payment receipt
...
The product needs a consistent, authorized way to **generate payment receipt**. Success means: Authorized participants receive an immutable receipt snapshot. The scenario starts when an eligible buyer initiates the action from `/orders/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/orders/:id/receipt`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Payment module validates the business invariant for **generate payment receipt** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from orders, payment_intents, settlements.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Authorized participants receive an immutable receipt snapshot.
...
- Emit `payments.generate-payment-receipt.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/orders/:id/receipt`.
...
`query generatePaymentReceipt` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Authorized participants receive an immutable receipt snapshot.
...
- Integration: `GET /api/v1/orders/:id/receipt`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
