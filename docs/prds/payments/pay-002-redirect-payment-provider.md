# PAY-002: Redirect to payment provider
...
**Current evidence:** No provider selected.

## Problem and outcome

The product needs a consistent, authorized way to **redirect to payment provider**. Success means: The buyer enters a provider-owned payment surface with signed return state. The scenario starts when an eligible buyer initiates the action from `/checkout/:listingId` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `provider redirect or SDK`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Payment module validates the business invariant for **redirect to payment provider** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across payment_intents.
...
6. The client renders the canonical result and the next valid action. The buyer enters a provider-owned payment surface with signed return state.
...
- Emit `payments.redirect-payment-provider.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `provider redirect or SDK`.
...
`mutation redirectPaymentProvider` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The buyer enters a provider-owned payment surface with signed return state.
...
- Integration: `provider redirect or SDK`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
