# PAY-007: Ignore duplicate payment event
...
The product needs a consistent, authorized way to **ignore duplicate payment event**. Success means: Repeated provider event IDs return success without repeated side effects. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `WebhookDeduplicationPolicy`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Payment module validates the business invariant for **ignore duplicate payment event** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across payment_events.
...
6. The worker records the canonical result; dependent clients observe it through their owning query or event. Repeated provider event IDs return success without repeated side effects.
...
- Emit `payments.deduplicate-payment-event.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `WebhookDeduplicationPolicy`.
...
- Given valid prerequisites and authorization, the scenario ends with: Repeated provider event IDs return success without repeated side effects.
...
- Integration: `WebhookDeduplicationPolicy`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
