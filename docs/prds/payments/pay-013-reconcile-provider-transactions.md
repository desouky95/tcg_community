# PAY-013: Reconcile provider transactions
...
**Current evidence:** Planned payment reconciliation.

## Problem and outcome

The product needs a consistent, authorized way to **reconcile provider transactions**. Success means: Internal and provider totals are compared and mismatches queued. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PaymentReconciliationJob`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Payment module validates the business invariant for **reconcile provider transactions** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across payment_intents, payment_events, settlements, refunds.
...
6. The worker records the canonical result; dependent clients observe it through their owning query or event. Internal and provider totals are compared and mismatches queued.
...
- Emit `payments.reconcile-provider-transactions.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PaymentReconciliationJob`.
...
- Given valid prerequisites and authorization, the scenario ends with: Internal and provider totals are compared and mismatches queued.
...
- Integration: `PaymentReconciliationJob`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
