...
| Surface | `/verify-otp` |
| Dependencies | [AUTH-002](../identity/auth-002-request-signup-otp.md) |

**Current evidence:** apps/web/src/pages/VerifyOTP.tsx.

## Problem and outcome

The product needs a consistent, authorized way to **resend mobile otp**. Success means: A replacement challenge is issued subject to cooldown limits. The scenario starts when an eligible guest initiates the action from `/verify-otp` and ends when the canonical result is persisted or a typed terminal error is returned.
...
3. Identity module validates the business invariant for **resend mobile otp** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across otp_challenges.
...
6. The client renders the canonical result and the next valid action. A replacement challenge is issued subject to cooldown limits.
...
- Emit `identity.resend-mobile-otp.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation resendMobileOtp` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A replacement challenge is issued subject to cooldown limits.
