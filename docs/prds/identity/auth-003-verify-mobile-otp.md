...
| Actors | guest |
| Platforms | public web, mobile |
| Surface | `/verify-otp` |
| Dependencies | [AUTH-001](../identity/auth-001-register-account.md) |

**Current evidence:** apps/web/src/pages/VerifyOTP.tsx; apps/api AuthController.verifyOtp.

## Problem and outcome

The product needs a consistent, authorized way to **verify mobile otp**. Success means: The challenge is consumed, the account is verified, and a session is issued. The scenario starts when an eligible guest initiates the action from `/verify-otp` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/auth/verify-otp`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Identity module validates the business invariant for **verify mobile otp** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across otp_challenges, users, access_tokens.
5. The outbox publishes IdentityVerified, SessionRevoked, ProfileUpdated; noncritical notifications, projections, and analytics run asynchronously.
6. The client renders the canonical result and the next valid action. The challenge is consumed, the account is verified, and a session is issued.
...
- Emit `identity.verify-mobile-otp.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/auth/verify-otp`.
...
`mutation verifyMobileOtp` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The challenge is consumed, the account is verified, and a session is issued.
...
- Integration: `POST /api/v1/auth/verify-otp`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
