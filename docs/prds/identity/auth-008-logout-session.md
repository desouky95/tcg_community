# AUTH-008: Log out current session
...
**Current evidence:** apps/api AuthController.logout.

## Problem and outcome

The product needs a consistent, authorized way to **log out current session**. Success means: The current token is revoked and local credentials are cleared. The scenario starts when an eligible collector initiates the action from `/settings` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/auth/logout`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Identity module validates the business invariant for **log out current session** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. The current token is revoked and local credentials are cleared.
...
- Emit `identity.logout-session.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation logoutSession` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The current token is revoked and local credentials are cleared.
...
- Integration: `POST /api/v1/auth/logout`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
