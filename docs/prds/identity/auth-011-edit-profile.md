...
**Current evidence:** apps/web/src/pages/ProfileEdit.tsx.

## Problem and outcome

The product needs a consistent, authorized way to **edit profile**. Success means: Validated profile fields are persisted. The scenario starts when an eligible collector initiates the action from `/profile/edit` and ends when the canonical result is persisted or a typed terminal error is returned.
...
3. Identity module validates the business invariant for **edit profile** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across users.
...
6. The client renders the canonical result and the next valid action. Validated profile fields are persisted.
...
- Emit `identity.edit-profile.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation editProfile` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Validated profile fields are persisted.
