# AUTH-013: View public collector profile
...
**Current evidence:** apps/web/src/pages/Profile.tsx; apps/api ProfileController.show.

## Problem and outcome

The product needs a consistent, authorized way to **view public collector profile**. Success means: Only public collector information is returned. The scenario starts when an eligible collector initiates the action from `/profile/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/users/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Identity module validates the business invariant for **view public collector profile** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from users, reputation_snapshots.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Only public collector information is returned.
...
- Emit `identity.view-public-profile.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`query viewPublicProfile` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Only public collector information is returned.
...
- Integration: `GET /api/v1/users/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
