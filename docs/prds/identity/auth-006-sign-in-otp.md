...
**Current evidence:** No dedicated route.

## Problem and outcome

The product needs a consistent, authorized way to **sign in with otp**. Success means: A consumed login challenge creates a session without a password. The scenario starts when an eligible guest initiates the action from `/login` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/auth/otp-login`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Identity module validates the business invariant for **sign in with otp** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across otp_challenges, access_tokens.
...
6. The client renders the canonical result and the next valid action. A consumed login challenge creates a session without a password.
...
- Emit `identity.sign-in-otp.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/auth/otp-login`.
...
`mutation signInOtp` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A consumed login challenge creates a session without a password.
...
- Integration: `POST /api/v1/auth/otp-login`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
