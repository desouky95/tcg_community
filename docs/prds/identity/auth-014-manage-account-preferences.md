# AUTH-014: Manage account preferences
...
| Surface | `/settings` |
| Dependencies | [AUTH-010](../identity/auth-010-view-own-profile.md) |

**Current evidence:** Planned /settings route.

## Problem and outcome

The product needs a consistent, authorized way to **manage account preferences**. Success means: Language, theme, privacy, and communication settings are saved. The scenario starts when an eligible collector initiates the action from `/settings` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PUT /api/v1/account/preferences`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Identity module validates the business invariant for **manage account preferences** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across users, notification_preferences.
...
6. The client renders the canonical result and the next valid action. Language, theme, privacy, and communication settings are saved.
...
- Emit `identity.manage-account-preferences.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PUT /api/v1/account/preferences`.
...
`mutation manageAccountPreferences` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Language, theme, privacy, and communication settings are saved.
...
- Integration: `PUT /api/v1/account/preferences`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
