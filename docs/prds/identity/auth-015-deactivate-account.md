...
The product needs a consistent, authorized way to **deactivate account**. Success means: Login is disabled while legally required records remain retained. The scenario starts when an eligible collector initiates the action from `/settings` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/account/deactivate`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Identity module validates the business invariant for **deactivate account** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Login is disabled while legally required records remain retained.
...
- Emit `identity.deactivate-account.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/account/deactivate`.
...
`mutation deactivateAccount` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Login is disabled while legally required records remain retained.
...
- Integration: `POST /api/v1/account/deactivate`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
