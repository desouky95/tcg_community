# AUTH-005: Sign in with password
...
**Current evidence:** apps/web/src/pages/Login.tsx; apps/api AuthController.login.

## Problem and outcome

The product needs a consistent, authorized way to **sign in with password**. Success means: Valid credentials create an authenticated session. The scenario starts when an eligible guest initiates the action from `/login` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/auth/login`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Identity module validates the business invariant for **sign in with password** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Valid credentials create an authenticated session.
...
- Emit `identity.sign-in-password.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation signInPassword` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Valid credentials create an authenticated session.
...
- Integration: `POST /api/v1/auth/login`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
