...
**Current evidence:** apps/api AuthController.requestOtp.

## Problem and outcome

The product needs a consistent, authorized way to **request signup otp**. Success means: A bounded-lifetime verification challenge is issued. The scenario starts when an eligible guest initiates the action from `/signup` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/auth/request-otp`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Identity module validates the business invariant for **request signup otp** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across otp_challenges, users.
...
6. The client renders the canonical result and the next valid action. A bounded-lifetime verification challenge is issued.
...
- Emit `identity.request-signup-otp.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/auth/request-otp`.
...
`mutation requestSignupOtp` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A bounded-lifetime verification challenge is issued.
...
- Integration: `POST /api/v1/auth/request-otp`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
