# NTF-007: Retry failed notification
...
| Dependencies | [NTF-004](../notifications/ntf-004-deliver-realtime-notification.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **retry failed notification**. Success means: Transient failures retry with exponential backoff and jitter. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `NotificationDeliveryJob`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Notification module validates the business invariant for **retry failed notification** and rejects illegal or stale state without side effects.
...
6. The worker records the canonical result; dependent clients observe it through their owning query or event. Transient failures retry with exponential backoff and jitter.
...
- Emit `notifications.retry-failed-notification.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `NotificationDeliveryJob`.
...
- Given valid prerequisites and authorization, the scenario ends with: Transient failures retry with exponential backoff and jitter.
...
- Integration: `NotificationDeliveryJob`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
