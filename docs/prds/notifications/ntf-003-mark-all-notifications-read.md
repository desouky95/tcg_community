# NTF-003: Mark all notifications read
...
The product needs a consistent, authorized way to **mark all notifications read**. Success means: All current notifications become read using a bounded cutoff. The scenario starts when an eligible collector initiates the action from `/notifications` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/notifications/read-all`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Notification module validates the business invariant for **mark all notifications read** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across notifications.
...
6. The client renders the canonical result and the next valid action. All current notifications become read using a bounded cutoff.
...
- Emit `notifications.mark-all-notifications-read.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/notifications/read-all`.
...
`mutation markAllNotificationsRead` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: All current notifications become read using a bounded cutoff.
...
- Integration: `POST /api/v1/notifications/read-all`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
