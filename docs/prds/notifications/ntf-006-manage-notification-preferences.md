# NTF-006: Manage notification preferences
...
| Surface | `/settings` |
| Dependencies | [AUTH-014](../identity/auth-014-manage-account-preferences.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **manage notification preferences**. Success means: Per-event channel preferences are persisted with mandatory exceptions. The scenario starts when an eligible collector initiates the action from `/settings` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PUT /api/v1/notifications/preferences`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Notification module validates the business invariant for **manage notification preferences** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across notification_preferences.
...
6. The client renders the canonical result and the next valid action. Per-event channel preferences are persisted with mandatory exceptions.
...
- Emit `notifications.manage-notification-preferences.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PUT /api/v1/notifications/preferences`.
...
`mutation manageNotificationPreferences` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Per-event channel preferences are persisted with mandatory exceptions.
...
- Integration: `PUT /api/v1/notifications/preferences`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
