# NTF-005: Register mobile push device
...
| Dependencies | [MOB-003](../mobile/mob-003-request-push-permission.md) |

**Current evidence:** No mobile implementation.

## Problem and outcome

The product needs a consistent, authorized way to **register mobile push device**. Success means: The authenticated installation owns one rotated push token. The scenario starts when an eligible collector initiates the action from `mobile settings` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GraphQL registerDevice`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Notification module validates the business invariant for **register mobile push device** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. The authenticated installation owns one rotated push token.
...
- Emit `notifications.register-push-device.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GraphQL registerDevice`.
...
`mutation registerPushDevice` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The authenticated installation owns one rotated push token.
...
- Integration: `GraphQL registerDevice`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
