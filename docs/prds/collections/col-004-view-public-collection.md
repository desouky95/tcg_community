# COL-004: View public collection
...
| Dependencies | [COL-001](../collections/col-001-start-collection.md) |

**Current evidence:** apps/web/src/pages/Collection.tsx.

## Problem and outcome

The product needs a consistent, authorized way to **view public collection**. Success means: A visibility-filtered collection is shown without private notes. The scenario starts when an eligible guest initiates the action from `/collection/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/collections/:id/public`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Collections module validates the business invariant for **view public collection** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from user_checklists, collection_entries.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. A visibility-filtered collection is shown without private notes.
...
- Emit `collections.view-public-collection.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/collections/:id/public`.
...
`query viewPublicCollection` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A visibility-filtered collection is shown without private notes.
...
- Integration: `GET /api/v1/collections/:id/public`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
