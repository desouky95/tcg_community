...
**Current evidence:** apps/web/src/pages/CollectionEdit.tsx.

## Problem and outcome

The product needs a consistent, authorized way to **update card ownership**. Success means: Owned quantity changes atomically. The scenario starts when an eligible collector initiates the action from `/collection/:id/edit` and ends when the canonical result is persisted or a typed terminal error is returned.
...
3. Collections module validates the business invariant for **update card ownership** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Owned quantity changes atomically.
...
- Emit `collections.update-card-ownership.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation updateCardOwnership` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Owned quantity changes atomically.
