# COL-001: Start collection from checklist
...
**Current evidence:** apps/web/src/pages/Dashboard.tsx.

## Problem and outcome

The product needs a consistent, authorized way to **start collection from checklist**. Success means: A single owned collection is created for the checklist. The scenario starts when an eligible collector initiates the action from `/dashboard` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/user-checklists/:checklistId`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Collections module validates the business invariant for **start collection from checklist** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. A single owned collection is created for the checklist.
...
- Emit `collections.start-collection.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/user-checklists/:checklistId`.
...
`mutation startCollection` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A single owned collection is created for the checklist.
...
- Integration: `POST /api/v1/user-checklists/:checklistId`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
