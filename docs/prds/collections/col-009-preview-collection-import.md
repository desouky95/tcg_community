# COL-009: Preview collection import
...
**Current evidence:** Import exists without explicit preview contract.

## Problem and outcome

The product needs a consistent, authorized way to **preview collection import**. Success means: The user receives row-level validation before mutation. The scenario starts when an eligible collector initiates the action from `/collection/:id/edit` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/user-checklists/:id/import?dryRun=true`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Collections module validates the business invariant for **preview collection import** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. The user receives row-level validation before mutation.
...
- Emit `collections.preview-collection-import.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/user-checklists/:id/import?dryRun=true`.
...
`mutation previewCollectionImport` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The user receives row-level validation before mutation.
...
- Integration: `POST /api/v1/user-checklists/:id/import?dryRun=true`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
