# COL-012: Manage collection visibility
...
| Surface | `collection settings` |
| Dependencies | [COL-003](../collections/col-003-view-own-collection.md) |

**Current evidence:** Public route exists; visibility controls are incomplete.

## Problem and outcome

The product needs a consistent, authorized way to **manage collection visibility**. Success means: Public, unlisted, or private visibility is enforced consistently. The scenario starts when an eligible collector initiates the action from `collection settings` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PUT /api/v1/user-checklists/:id/visibility`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Collections module validates the business invariant for **manage collection visibility** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across user_checklists.
...
6. The client renders the canonical result and the next valid action. Public, unlisted, or private visibility is enforced consistently.
...
- Emit `collections.manage-collection-visibility.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PUT /api/v1/user-checklists/:id/visibility`.
...
`mutation manageCollectionVisibility` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Public, unlisted, or private visibility is enforced consistently.
...
- Integration: `PUT /api/v1/user-checklists/:id/visibility`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
