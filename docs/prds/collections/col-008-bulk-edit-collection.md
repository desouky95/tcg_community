...
**Current evidence:** Current UI batches changes without explicit version contract.

## Problem and outcome

The product needs a consistent, authorized way to **bulk edit collection**. Success means: A version-checked batch succeeds atomically. The scenario starts when an eligible collector initiates the action from `/collection/:id/edit` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PATCH /api/v1/user-checklists/:id/entries`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Collections module validates the business invariant for **bulk edit collection** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. A version-checked batch succeeds atomically.
...
- Emit `collections.bulk-edit-collection.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PATCH /api/v1/user-checklists/:id/entries`.
...
`mutation bulkEditCollection` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A version-checked batch succeeds atomically.
...
- Integration: `PATCH /api/v1/user-checklists/:id/entries`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
