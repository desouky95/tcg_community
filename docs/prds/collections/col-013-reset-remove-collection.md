# COL-013: Reset or remove collection
...
The product needs a consistent, authorized way to **reset or remove collection**. Success means: A confirmed reset removes active progress while preserving audit obligations. The scenario starts when an eligible collector initiates the action from `collection settings` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `DELETE /api/v1/user-checklists/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Collections module validates the business invariant for **reset or remove collection** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across user_checklists, collection_entries.
...
6. The client renders the canonical result and the next valid action. A confirmed reset removes active progress while preserving audit obligations.
...
- Emit `collections.reset-remove-collection.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `DELETE /api/v1/user-checklists/:id`.
...
`mutation resetRemoveCollection` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A confirmed reset removes active progress while preserving audit obligations.
...
- Integration: `DELETE /api/v1/user-checklists/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
