# COL-006: Track duplicate or offered cards
...
| Dependencies | [COL-005](../collections/col-005-update-card-ownership.md) |

**Current evidence:** Existing user checklist update.

## Problem and outcome

The product needs a consistent, authorized way to **track duplicate or offered cards**. Success means: Offered quantities remain nonnegative and do not exceed owned quantity. The scenario starts when an eligible collector initiates the action from `/collection/:id/edit` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/user-checklists/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Collections module validates the business invariant for **track duplicate or offered cards** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Offered quantities remain nonnegative and do not exceed owned quantity.
...
- Emit `collections.track-duplicates-offers.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/user-checklists/:id`.
...
`mutation trackDuplicatesOffers` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Offered quantities remain nonnegative and do not exceed owned quantity.
...
- Integration: `POST /api/v1/user-checklists/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
