...
**Current evidence:** apps/api UserChecklistsController.show.

## Problem and outcome

The product needs a consistent, authorized way to **view own collection**. Success means: The owner sees editable card states. The scenario starts when an eligible collector initiates the action from `/collection/:id/edit` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/user-checklists/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Collections module validates the business invariant for **view own collection** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. The owner sees editable card states.
...
- Emit `collections.view-own-collection.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/user-checklists/:id`.
...
`query viewOwnCollection` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The owner sees editable card states.
...
- Integration: `GET /api/v1/user-checklists/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
