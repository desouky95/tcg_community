...
The product needs a consistent, authorized way to **export collection**. Success means: The owner downloads a versioned export. The scenario starts when an eligible collector initiates the action from `collection actions` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/user-checklists/:id/export`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Collections module validates the business invariant for **export collection** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. The owner downloads a versioned export.
...
- Emit `collections.export-collection.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/user-checklists/:id/export`.
...
`query exportCollection` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The owner downloads a versioned export.
...
- Integration: `GET /api/v1/user-checklists/:id/export`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
