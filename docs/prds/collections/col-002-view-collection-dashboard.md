# COL-002: View collection dashboard
...
The product needs a consistent, authorized way to **view collection dashboard**. Success means: Collection progress summaries are shown. The scenario starts when an eligible collector initiates the action from `/dashboard` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/account/collections`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Collections module validates the business invariant for **view collection dashboard** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from user_checklists.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Collection progress summaries are shown.
...
- Emit `collections.view-collection-dashboard.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/account/collections`.
...
`query viewCollectionDashboard` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Collection progress summaries are shown.
...
- Integration: `GET /api/v1/account/collections`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
