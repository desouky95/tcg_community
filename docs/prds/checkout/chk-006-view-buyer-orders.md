...
The product needs a consistent, authorized way to **view buyer orders**. Success means: The buyer sees paginated current and historical orders. The scenario starts when an eligible buyer initiates the action from `orders list` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/orders`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Checkout module validates the business invariant for **view buyer orders** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from orders, order_items.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. The buyer sees paginated current and historical orders.
...
- Emit `checkout.view-buyer-orders.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`query viewBuyerOrders` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The buyer sees paginated current and historical orders.
...
- Integration: `GET /api/v1/orders`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
