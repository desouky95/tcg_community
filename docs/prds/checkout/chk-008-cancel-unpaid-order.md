...
The product needs a consistent, authorized way to **cancel unpaid order**. Success means: A pending-payment order cancels and releases inventory. The scenario starts when an eligible buyer initiates the action from `/orders/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/orders/:id/cancel`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Checkout module validates the business invariant for **cancel unpaid order** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across orders, listing_reservations.
...
6. The client renders the canonical result and the next valid action. A pending-payment order cancels and releases inventory.
...
- Emit `checkout.cancel-unpaid-order.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation cancelUnpaidOrder` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A pending-payment order cancels and releases inventory.
...
- Integration: `POST /api/v1/orders/:id/cancel`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
