# CHK-002: Reserve listing quantity
...
| Dependencies | [CHK-001](../checkout/chk-001-check-listing-availability.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **reserve listing quantity**. Success means: Inventory is reserved atomically for a bounded TTL. The scenario starts when an eligible buyer initiates the action from `/checkout/:listingId` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/checkout/reservations`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Checkout module validates the business invariant for **reserve listing quantity** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Inventory is reserved atomically for a bounded TTL.
...
- Emit `checkout.reserve-listing-quantity.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/checkout/reservations`.
...
`mutation reserveListingQuantity` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Inventory is reserved atomically for a bounded TTL.
...
- Integration: `POST /api/v1/checkout/reservations`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
