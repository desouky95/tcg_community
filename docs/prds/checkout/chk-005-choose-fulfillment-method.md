# CHK-005: Choose contact-led fulfillment
...
The product needs a consistent, authorized way to **choose contact-led fulfillment**. Success means: The buyer selects meetup or self-arranged shipping and accepts liability text. The scenario starts when an eligible buyer initiates the action from `/checkout/:listingId` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PUT /api/v1/checkout/orders/:id/fulfillment`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Checkout module validates the business invariant for **choose contact-led fulfillment** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across orders, fulfillments.
5. The outbox publishes ListingReserved, OrderCreated, OrderCancelled, ContactReleased; noncritical notifications, projections, and analytics run asynchronously.
6. The client renders the canonical result and the next valid action. The buyer selects meetup or self-arranged shipping and accepts liability text.
...
- Emit `checkout.choose-fulfillment-method.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PUT /api/v1/checkout/orders/:id/fulfillment`.
...
`mutation chooseFulfillmentMethod` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The buyer selects meetup or self-arranged shipping and accepts liability text.
...
- Integration: `PUT /api/v1/checkout/orders/:id/fulfillment`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
