# CHK-004: Submit checkout details
...
The product needs a consistent, authorized way to **submit checkout details**. Success means: A price-snapshotted pending-payment order is created. The scenario starts when an eligible buyer initiates the action from `/checkout/:listingId` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/checkout/orders`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Checkout module validates the business invariant for **submit checkout details** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across orders, order_items, listing_reservations.
...
6. The client renders the canonical result and the next valid action. A price-snapshotted pending-payment order is created.
...
- Emit `checkout.submit-checkout-details.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/checkout/orders`.
...
`mutation submitCheckoutDetails` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A price-snapshotted pending-payment order is created.
...
- Integration: `POST /api/v1/checkout/orders`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
