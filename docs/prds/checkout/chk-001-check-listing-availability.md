# CHK-001: Check listing availability
...
| Dependencies | [LIST-003](../marketplace/list-003-view-listing-detail.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **check listing availability**. Success means: The buyer receives current purchasable quantity and price version. The scenario starts when an eligible buyer initiates the action from `/checkout/:listingId` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/checkout/listings/:id/availability`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Checkout module validates the business invariant for **check listing availability** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from listings, listing_reservations.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. The buyer receives current purchasable quantity and price version.
...
- Emit `checkout.check-listing-availability.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/checkout/listings/:id/availability`.
...
`query checkListingAvailability` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The buyer receives current purchasable quantity and price version.
...
- Integration: `GET /api/v1/checkout/listings/:id/availability`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
