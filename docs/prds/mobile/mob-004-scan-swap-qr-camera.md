# MOB-004: Scan swap QR with camera
...
| Platforms | mobile |
| Surface | `native swap detail` |
| Dependencies | [SWAP-014](../swaps/swap-014-scan-handoff-qr.md) |

**Current evidence:** React Native app not present.

## Problem and outcome

The product needs a consistent, authorized way to **scan swap qr with camera**. Success means: Camera output is verified as a signed unexpired handoff token. The scenario starts when an eligible collector initiates the action from `native swap detail` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GraphQL scanSwapQr`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Mobile client and shared-contract layer validates the business invariant for **scan swap qr with camera** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across swap_handoffs.
...
6. The client renders the canonical result and the next valid action. Camera output is verified as a signed unexpired handoff token.
...
- Emit `mobile.scan-swap-qr-camera.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GraphQL scanSwapQr`.
...
`mutation scanSwapQrCamera` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Camera output is verified as a signed unexpired handoff token.
...
- Integration: `GraphQL scanSwapQr`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
