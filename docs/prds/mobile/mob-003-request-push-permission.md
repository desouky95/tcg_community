# MOB-003: Request push permission
...
| Domain | React Native |
| Actors | collector, buyer, seller |
| Platforms | mobile |
| Surface | `native onboarding/settings` |
| Dependencies | [MOB-001](../mobile/mob-001-authenticate-mobile-collector.md) |

**Current evidence:** React Native app not present.

## Problem and outcome

The product needs a consistent, authorized way to **request push permission**. Success means: Permission outcome is respected and the token is registered only with consent. The scenario starts when an eligible collector initiates the action from `native onboarding/settings` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `native OS permission and GraphQL registerDevice`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Mobile client and shared-contract layer validates the business invariant for **request push permission** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across device_installations.
5. The outbox publishes DeviceRegistered, MobileSyncRequested, MinimumVersionChanged; noncritical notifications, projections, and analytics run asynchronously.
6. The client renders the canonical result and the next valid action. Permission outcome is respected and the token is registered only with consent.
...
- Emit `mobile.request-push-permission.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `native OS permission and GraphQL registerDevice`.
...
`mutation requestPushPermission` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.

## Backend and data rules

- Owning component: Mobile client and shared-contract layer; other domains interact through application interfaces and outbox events, not direct cross-domain writes.
- Authoritative records: device_installations(id, user_id, platform, push_token_hash, app_version, last_seen_at); mobile_sync_cursors(user_id, resource_type, cursor, updated_at).
- Required indexes and constraints: unique device_installations(platform, push_token_hash); device_installations(user_id, last_seen_at); unique mobile_sync_cursors(user_id, resource_type).
- Offline data lives in encrypted device storage; server responses follow domain cache policies.
...
- Given valid prerequisites and authorization, the scenario ends with: Permission outcome is respected and the token is registered only with consent.
...
- Integration: `native OS permission and GraphQL registerDevice`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
