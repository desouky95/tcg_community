# MOB-005: Upload mobile listing media
...
| Platforms | mobile |
| Surface | `native listing editor` |
| Dependencies | [LIST-008](../marketplace/list-008-upload-listing-media.md) |

**Current evidence:** React Native app not present.

## Problem and outcome

The product needs a consistent, authorized way to **upload mobile listing media**. Success means: Compressed media uploads resumably and is server-validated. The scenario starts when an eligible collector initiates the action from `native listing editor` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GraphQL createListingMediaUpload`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Mobile client and shared-contract layer validates the business invariant for **upload mobile listing media** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Compressed media uploads resumably and is server-validated.
...
- Emit `mobile.upload-mobile-listing-media.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GraphQL createListingMediaUpload`.
...
`mutation uploadMobileListingMedia` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Compressed media uploads resumably and is server-validated.
...
- Integration: `GraphQL createListingMediaUpload`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
