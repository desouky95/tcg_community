...
| Surface | `collection and search surfaces` |
| Dependencies | None. |
...
The product needs a consistent, authorized way to **render empty result**. Success means: The user sees why no records exist and the next useful action. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `successful empty response`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Platform edge and client state layer validates the business invariant for **render empty result** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. The user sees why no records exist and the next useful action.
...
- Emit `system.render-empty-state.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation renderEmptyState` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The user sees why no records exist and the next useful action.
...
- Integration: `successful empty response`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
