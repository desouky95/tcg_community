# SYS-014: View product landing page
...
| Actors | guest |
| Platforms | public web |
| Surface | `/` |
| Dependencies | None. |

**Current evidence:** apps/web/src/pages/Landing.tsx.

## Problem and outcome

The product needs a consistent, authorized way to **view product landing page**. Success means: A guest understands the product and can reach catalogue, marketplace, sign-in, or signup. The scenario starts when an eligible guest initiates the action from `/` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `client-side route`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Platform edge and client state layer validates the business invariant for **view product landing page** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from none.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. A guest understands the product and can reach catalogue, marketplace, sign-in, or signup.
...
- Emit `system.view-product-landing.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `client-side route`.
...
`query viewProductLanding` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A guest understands the product and can reach catalogue, marketplace, sign-in, or signup.
...
- Integration: `client-side route`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
