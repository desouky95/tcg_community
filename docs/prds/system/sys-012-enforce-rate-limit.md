...
**Current evidence:** No explicit rate limiter.

## Problem and outcome

The product needs a consistent, authorized way to **enforce rate limit**. Success means: Token-bucket limits return Retry-After and protect sensitive actions. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `HTTP 429 contract`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Platform edge and client state layer validates the business invariant for **enforce rate limit** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Token-bucket limits return Retry-After and protect sensitive actions.
...
- Emit `system.enforce-rate-limit.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `HTTP 429 contract`.
...
`mutation enforceRateLimit` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Token-bucket limits return Retry-After and protect sensitive actions.
...
- Integration: `HTTP 429 contract`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
