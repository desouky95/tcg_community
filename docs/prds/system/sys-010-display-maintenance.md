# SYS-010: Display maintenance state
...
| Surface | `all clients` |
| Dependencies | [ADM-012](../administration/adm-012-activate-maintenance-mode.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **display maintenance state**. Success means: Clients display localized timing and preserve safe read-only behavior. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `HTTP 503 maintenance contract`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Platform edge and client state layer validates the business invariant for **display maintenance state** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across maintenance_windows.
...
6. The client renders the canonical result and the next valid action. Clients display localized timing and preserve safe read-only behavior.
...
- Emit `system.display-maintenance.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `HTTP 503 maintenance contract`.
...
`mutation displayMaintenance` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Clients display localized timing and preserve safe read-only behavior.
...
- Integration: `HTTP 503 maintenance contract`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
