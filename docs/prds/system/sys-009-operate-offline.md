...
**Current evidence:** No shared offline policy.

## Problem and outcome

The product needs a consistent, authorized way to **operate offline**. Success means: Clients distinguish cached reads from queued or blocked writes. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `network state contract`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Platform edge and client state layer validates the business invariant for **operate offline** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across mobile_sync_cursors.
...
6. The client renders the canonical result and the next valid action. Clients distinguish cached reads from queued or blocked writes.
...
- Emit `system.operate-offline.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `network state contract`.
...
`mutation operateOffline` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Clients distinguish cached reads from queued or blocked writes.
...
- Integration: `network state contract`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
