...
**Current evidence:** Implemented inconsistently.

## Problem and outcome

The product needs a consistent, authorized way to **render loading state**. Success means: Longer reads show stable accessible skeletons without layout shift. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `client state contract`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Platform edge and client state layer validates the business invariant for **render loading state** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Longer reads show stable accessible skeletons without layout shift.
...
- Emit `system.render-loading-state.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `client state contract`.
...
`mutation renderLoadingState` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Longer reads show stable accessible skeletons without layout shift.
...
- Integration: `client state contract`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
