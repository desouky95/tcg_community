# CAT-001: Browse catalogue categories
...
**Current evidence:** apps/web/src/pages/Checklists.tsx.

## Problem and outcome

The product needs a consistent, authorized way to **browse catalogue categories**. Success means: Published catalogue categories are listed. The scenario starts when an eligible guest initiates the action from `/checklists` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/categories`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **browse catalogue categories** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from categories.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Published catalogue categories are listed.
...
- Emit `catalogue.browse-categories.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`query browseCategories` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Published catalogue categories are listed.
...
- Integration: `GET /api/v1/categories`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
