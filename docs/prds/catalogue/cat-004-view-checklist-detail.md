...
| Surface | `/checklists/:id` |
| Dependencies | [CAT-003](../catalogue/cat-003-browse-checklists.md) |

**Current evidence:** apps/api ChecklistsController.show.

## Problem and outcome

The product needs a consistent, authorized way to **view checklist detail**. Success means: Checklist metadata and cards are returned. The scenario starts when an eligible guest initiates the action from `/checklists/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/checklists/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **view checklist detail** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from checklists, cards.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Checklist metadata and cards are returned.
...
- Emit `catalogue.view-checklist-detail.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/checklists/:id`.
...
`query viewChecklistDetail` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Checklist metadata and cards are returned.
...
- Integration: `GET /api/v1/checklists/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
