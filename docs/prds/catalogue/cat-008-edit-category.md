...
**Current evidence:** Admin UI exists; update contract incomplete.

## Problem and outcome

The product needs a consistent, authorized way to **edit category**. Success means: Category metadata changes with an audit record. The scenario starts when an eligible administrator initiates the action from `/admin/categories` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PUT /api/v1/categories/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **edit category** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Category metadata changes with an audit record.
...
- Emit `catalogue.edit-category.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PUT /api/v1/categories/:id`.
...
`mutation editCategory` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Category metadata changes with an audit record.
...
- Integration: `PUT /api/v1/categories/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
