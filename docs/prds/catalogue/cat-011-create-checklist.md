...
**Current evidence:** apps/api ChecklistsController.store.

## Problem and outcome

The product needs a consistent, authorized way to **create checklist**. Success means: A checklist is created under an authorized category. The scenario starts when an eligible administrator initiates the action from `/admin/collections` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/checklists`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **create checklist** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. A checklist is created under an authorized category.
...
- Emit `catalogue.create-checklist.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/checklists`.
...
`mutation createChecklist` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A checklist is created under an authorized category.
...
- Integration: `POST /api/v1/checklists`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
