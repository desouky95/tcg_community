...
**Current evidence:** apps/api ChecklistsController.destroy.

## Problem and outcome

The product needs a consistent, authorized way to **archive checklist**. Success means: The checklist is hidden while user records remain valid. The scenario starts when an eligible administrator initiates the action from `/admin/collections` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `DELETE /api/v1/checklists/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **archive checklist** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across checklists.
...
6. The client renders the canonical result and the next valid action. The checklist is hidden while user records remain valid.
...
- Emit `catalogue.archive-checklist.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `DELETE /api/v1/checklists/:id`.
...
`mutation archiveChecklist` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The checklist is hidden while user records remain valid.
...
- Integration: `DELETE /api/v1/checklists/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
