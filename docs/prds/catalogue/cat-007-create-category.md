...
**Current evidence:** apps/api CategoriesController.store.

## Problem and outcome

The product needs a consistent, authorized way to **create category**. Success means: An administrator creates a draft or published category. The scenario starts when an eligible administrator initiates the action from `/admin/categories` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/categories`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **create category** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. An administrator creates a draft or published category.
...
- Emit `catalogue.create-category.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation createCategory` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: An administrator creates a draft or published category.
...
- Integration: `POST /api/v1/categories`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
