...
| Surface | `card detail surface` |
| Dependencies | [CAT-004](../catalogue/cat-004-view-checklist-detail.md) |

**Current evidence:** Card data exists without a dedicated route.

## Problem and outcome

The product needs a consistent, authorized way to **view card detail**. Success means: Canonical card metadata and associated availability are shown. The scenario starts when an eligible guest initiates the action from `card detail surface` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/catalogue/cards/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **view card detail** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Canonical card metadata and associated availability are shown.
...
- Emit `catalogue.view-card-detail.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/catalogue/cards/:id`.
...
`query viewCardDetail` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Canonical card metadata and associated availability are shown.
...
- Integration: `GET /api/v1/catalogue/cards/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
