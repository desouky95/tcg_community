# CAT-015: Commit catalogue import
...
| Dependencies | [CAT-014](../catalogue/cat-014-validate-catalogue-import.md) |

**Current evidence:** Planned background import workflow.

## Problem and outcome

The product needs a consistent, authorized way to **commit catalogue import**. Success means: A validated import commits atomically and records its new catalogue version. The scenario starts when an eligible administrator initiates the action from `/admin/catalogue/import` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/admin/catalogue-imports/:id/commit`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **commit catalogue import** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. A validated import commits atomically and records its new catalogue version.
...
- Emit `catalogue.commit-catalogue-import.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/admin/catalogue-imports/:id/commit`.
...
`mutation commitCatalogueImport` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A validated import commits atomically and records its new catalogue version.
...
- Integration: `POST /api/v1/admin/catalogue-imports/:id/commit`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
