# CAT-014: Validate catalogue import
...
| Dependencies | [CAT-011](../catalogue/cat-011-create-checklist.md) |

**Current evidence:** Planned admin route.

## Problem and outcome

The product needs a consistent, authorized way to **validate catalogue import**. Success means: A dry-run report identifies invalid and conflicting rows. The scenario starts when an eligible administrator initiates the action from `/admin/catalogue/import` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/admin/catalogue-imports/validate`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **validate catalogue import** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across catalogue_imports.
...
6. The client renders the canonical result and the next valid action. A dry-run report identifies invalid and conflicting rows.
...
- Emit `catalogue.validate-catalogue-import.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/admin/catalogue-imports/validate`.
...
`mutation validateCatalogueImport` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A dry-run report identifies invalid and conflicting rows.
...
- Integration: `POST /api/v1/admin/catalogue-imports/validate`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
