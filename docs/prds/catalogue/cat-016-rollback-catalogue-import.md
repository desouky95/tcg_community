# CAT-016: Roll back catalogue import
...
| Platforms | admin web |
| Surface | `/admin/catalogue/import` |
| Dependencies | [CAT-015](../catalogue/cat-015-commit-catalogue-import.md) |

**Current evidence:** Planned background import workflow.

## Problem and outcome

The product needs a consistent, authorized way to **roll back catalogue import**. Success means: An eligible import restores the prior catalogue version through an auditable compensating change. The scenario starts when an eligible administrator initiates the action from `/admin/catalogue/import` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/admin/catalogue-imports/:id/rollback`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **roll back catalogue import** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across catalogue_imports, categories, checklists, cards.
5. The outbox publishes CatalogueChanged, CatalogueImportCompleted; noncritical notifications, projections, and analytics run asynchronously.
6. The client renders the canonical result and the next valid action. An eligible import restores the prior catalogue version through an auditable compensating change.
...
- Emit `catalogue.rollback-catalogue-import.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/admin/catalogue-imports/:id/rollback`.
...
`mutation rollbackCatalogueImport` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.

## Backend and data rules

- Owning component: Catalogue module; other domains interact through application interfaces and outbox events, not direct cross-domain writes.
- Authoritative records: categories(id, parent_id, name_en, name_ar, slug, status); checklists(id, category_id, name, version, status); cards(id, checklist_id, number, name, metadata, image_key, status); catalogue_imports(id, actor_id, checksum, status, summary).
- Required indexes and constraints: unique category/checklist slugs; unique cards(checklist_id, number); searchable card names; catalogue_imports(checksum).
- Public catalogue reads use cache-aside Redis with 15-minute TTL and version-key invalidation.
...
- Given valid prerequisites and authorization, the scenario ends with: An eligible import restores the prior catalogue version through an auditable compensating change.
...
- Integration: `POST /api/v1/admin/catalogue-imports/:id/rollback`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
