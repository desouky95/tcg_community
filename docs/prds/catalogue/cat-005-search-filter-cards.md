# CAT-005: Search and filter cards
...
| Domain | Catalogue |
| Actors | guest, collector |
| Platforms | public web, collector web, mobile |
| Surface | `catalogue search` |
| Dependencies | [CAT-003](../catalogue/cat-003-browse-checklists.md) |

**Current evidence:** Client-side search exists; server search is incomplete.

## Problem and outcome

The product needs a consistent, authorized way to **search and filter cards**. Success means: Users receive paginated cards matching normalized filters. The scenario starts when an eligible guest initiates the action from `catalogue search` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/catalogue/cards`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **search and filter cards** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from cards, checklists.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Users receive paginated cards matching normalized filters.
...
- Emit `catalogue.search-filter-cards.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/catalogue/cards`.
...
`query searchFilterCards` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Users receive paginated cards matching normalized filters.
...
- Integration: `GET /api/v1/catalogue/cards`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
