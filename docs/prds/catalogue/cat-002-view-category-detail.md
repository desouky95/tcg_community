...
| Surface | `/s/:categoryId` |
| Dependencies | [CAT-001](../catalogue/cat-001-browse-categories.md) |

**Current evidence:** apps/web/src/pages/CategoryDetail.tsx.

## Problem and outcome

The product needs a consistent, authorized way to **view category detail**. Success means: The category and its published checklists are shown. The scenario starts when an eligible guest initiates the action from `/s/:categoryId` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/categories/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **view category detail** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from categories, checklists.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. The category and its published checklists are shown.
...
- Emit `catalogue.view-category-detail.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`query viewCategoryDetail` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The category and its published checklists are shown.
...
- Integration: `GET /api/v1/categories/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
