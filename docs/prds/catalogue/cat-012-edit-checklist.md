...
**Current evidence:** apps/api ChecklistsController.update.

## Problem and outcome

The product needs a consistent, authorized way to **edit checklist**. Success means: Checklist metadata and version are updated. The scenario starts when an eligible administrator initiates the action from `/admin/collections` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PUT /api/v1/checklists/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **edit checklist** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Checklist metadata and version are updated.
...
- Emit `catalogue.edit-checklist.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PUT /api/v1/checklists/:id`.
...
`mutation editChecklist` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Checklist metadata and version are updated.
...
- Integration: `PUT /api/v1/checklists/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
