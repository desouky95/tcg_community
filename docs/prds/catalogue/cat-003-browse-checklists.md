...
| Surface | `/checklists` |
| Dependencies | [CAT-001](../catalogue/cat-001-browse-categories.md) |

**Current evidence:** apps/api ChecklistsController.index.

## Problem and outcome

The product needs a consistent, authorized way to **browse checklists**. Success means: Published checklists can be paged and filtered. The scenario starts when an eligible guest initiates the action from `/checklists` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/checklists`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Catalogue module validates the business invariant for **browse checklists** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from checklists, categories.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Published checklists can be paged and filtered.
...
- Emit `catalogue.browse-checklists.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/checklists`.
...
`query browseChecklists` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Published checklists can be paged and filtered.
...
- Integration: `GET /api/v1/checklists`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
