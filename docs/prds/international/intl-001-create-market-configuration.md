# INTL-001: Create market configuration
...
| Dependencies | [ADM-004](../administration/adm-004-assign-user-role.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **create market configuration**. Success means: A disabled country configuration is created with explicit defaults. The scenario starts when an eligible administrator initiates the action from `market admin` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/markets`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Market configuration module validates the business invariant for **create market configuration** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. A disabled country configuration is created with explicit defaults.
...
- Emit `international.create-market-configuration.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/markets`.
...
`mutation createMarketConfiguration` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A disabled country configuration is created with explicit defaults.
...
- Integration: `POST /api/v1/markets`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
