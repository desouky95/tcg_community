# INTL-006: Refresh exchange rates
...
| Dependencies | [INTL-003](../international/intl-003-configure-market-currency.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **refresh exchange rates**. Success means: Signed source rates are captured without rewriting historical orders. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `ExchangeRateRefreshJob`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Market configuration module validates the business invariant for **refresh exchange rates** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across exchange_rates.
...
6. The worker records the canonical result; dependent clients observe it through their owning query or event. Signed source rates are captured without rewriting historical orders.
...
- Emit `international.refresh-exchange-rates.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `ExchangeRateRefreshJob`.
...
- Given valid prerequisites and authorization, the scenario ends with: Signed source rates are captured without rewriting historical orders.
...
- Integration: `ExchangeRateRefreshJob`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
