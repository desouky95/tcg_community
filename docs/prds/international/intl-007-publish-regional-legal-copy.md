# INTL-007: Publish regional legal copy
...
The product needs a consistent, authorized way to **publish regional legal copy**. Success means: Versioned terms become effective at a future timestamp. The scenario starts when an eligible administrator initiates the action from `market admin` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/markets/:id/legal-documents`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Market configuration module validates the business invariant for **publish regional legal copy** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across legal_documents.
...
6. The client renders the canonical result and the next valid action. Versioned terms become effective at a future timestamp.
...
- Emit `international.publish-regional-legal-copy.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/markets/:id/legal-documents`.
...
`mutation publishRegionalLegalCopy` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Versioned terms become effective at a future timestamp.
...
- Integration: `POST /api/v1/markets/:id/legal-documents`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
