...
| Dependencies | [REV-005](../reviews/rev-005-report-abusive-review.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **moderate review**. Success means: An authorized decision publishes, hides, or removes the review. The scenario starts when an eligible moderator initiates the action from `/admin/reports` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/admin/reviews/:id/decision`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Trust module validates the business invariant for **moderate review** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across reviews, review_reports, audit_logs.
...
6. The client renders the canonical result and the next valid action. An authorized decision publishes, hides, or removes the review.
...
- Emit `reviews.moderate-review.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/admin/reviews/:id/decision`.
...
`mutation moderateReview` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: An authorized decision publishes, hides, or removes the review.
...
- Integration: `POST /api/v1/admin/reviews/:id/decision`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
