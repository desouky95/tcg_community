# REV-002: Check review eligibility
...
**Current evidence:** Store validation exists; explicit eligibility endpoint absent.

## Problem and outcome

The product needs a consistent, authorized way to **check review eligibility**. Success means: Only completed counterparties can review once. The scenario starts when an eligible collector initiates the action from `/profile/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/reviews/eligibility`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Trust module validates the business invariant for **check review eligibility** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from reviews, swap_deals, orders.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Only completed counterparties can review once.
...
- Emit `reviews.check-review-eligibility.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/reviews/eligibility`.
...
`query checkReviewEligibility` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Only completed counterparties can review once.
...
- Integration: `GET /api/v1/reviews/eligibility`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
