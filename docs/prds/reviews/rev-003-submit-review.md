...
| Surface | `/profile/:id` |
| Dependencies | [REV-002](../reviews/rev-002-check-review-eligibility.md) |

**Current evidence:** apps/api ReviewsController.store.

## Problem and outcome

The product needs a consistent, authorized way to **submit review**. Success means: A validated review is linked to an eligible transaction. The scenario starts when an eligible collector initiates the action from `/profile/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/users/:targetUserId/reviews`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Trust module validates the business invariant for **submit review** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across reviews.
...
6. The client renders the canonical result and the next valid action. A validated review is linked to an eligible transaction.
...
- Emit `reviews.submit-review.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/users/:targetUserId/reviews`.
...
`mutation submitReview` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A validated review is linked to an eligible transaction.
...
- Integration: `POST /api/v1/users/:targetUserId/reviews`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
