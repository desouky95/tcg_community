...
| Dependencies | [REV-003](../reviews/rev-003-submit-review.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **remove review**. Success means: The author removes visible content while preserving transaction and moderation audit references. The scenario starts when an eligible collector initiates the action from `review actions` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `DELETE /api/v1/reviews/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Trust module validates the business invariant for **remove review** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across reviews, audit_logs.
5. The outbox publishes ReviewSubmitted, ReviewModerated, ReputationChanged; noncritical notifications, projections, and analytics run asynchronously.
6. The client renders the canonical result and the next valid action. The author removes visible content while preserving transaction and moderation audit references.
...
- Emit `reviews.remove-review.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `DELETE /api/v1/reviews/:id`.
...
`mutation removeReview` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.

## Backend and data rules

- Owning component: Trust module; other domains interact through application interfaces and outbox events, not direct cross-domain writes.
- Authoritative records: reviews(id, author_id, target_user_id, transaction_type, transaction_id, rating, body, status); reputation_snapshots(user_id, score, review_count, calculated_at); review_reports(id, review_id, reporter_id, reason, status).
- Required indexes and constraints: reviews(target_user_id, status, created_at); unique reviews(author_id, transaction_type, transaction_id); review_reports(status, created_at).
- Public reputation summaries may use a 5-minute cache invalidated by review or moderation events.
...
- Given valid prerequisites and authorization, the scenario ends with: The author removes visible content while preserving transaction and moderation audit references.
...
- Integration: `DELETE /api/v1/reviews/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
