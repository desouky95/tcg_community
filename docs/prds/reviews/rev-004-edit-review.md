...
The product needs a consistent, authorized way to **edit review**. Success means: The author updates an eligible review within the edit window and moderation policy. The scenario starts when an eligible collector initiates the action from `review actions` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `PATCH /api/v1/reviews/:id`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Trust module validates the business invariant for **edit review** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. The author updates an eligible review within the edit window and moderation policy.
...
- Emit `reviews.edit-review.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `PATCH /api/v1/reviews/:id`.
...
`mutation editReview` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The author updates an eligible review within the edit window and moderation policy.
...
- Integration: `PATCH /api/v1/reviews/:id`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
