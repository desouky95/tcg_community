...
| Dependencies | [REV-001](../reviews/rev-001-view-collector-reviews.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **report abusive review**. Success means: A unique open report enters moderation. The scenario starts when an eligible collector initiates the action from `review actions` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/reviews/:id/report`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Trust module validates the business invariant for **report abusive review** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across review_reports.
...
6. The client renders the canonical result and the next valid action. A unique open report enters moderation.
...
- Emit `reviews.report-abusive-review.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/reviews/:id/report`.
...
`mutation reportAbusiveReview` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A unique open report enters moderation.
...
- Integration: `POST /api/v1/reviews/:id/report`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
