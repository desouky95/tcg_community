# FUL-006: Seller confirms handoff
...
The product needs a consistent, authorized way to **seller confirms handoff**. Success means: The seller records handoff idempotently. The scenario starts when an eligible buyer initiates the action from `/orders/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/fulfillments/:id/confirm`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Fulfillment module validates the business invariant for **seller confirms handoff** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across fulfillment_confirmations.
...
6. The client renders the canonical result and the next valid action. The seller records handoff idempotently.
...
- Emit `fulfillment.seller-confirm-handoff.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/fulfillments/:id/confirm`.
...
`mutation sellerConfirmHandoff` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The seller records handoff idempotently.
...
- Integration: `POST /api/v1/fulfillments/:id/confirm`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
