# FUL-004: Update fulfillment status
...
The product needs a consistent, authorized way to **update fulfillment status**. Success means: An authorized participant records a legal progress state. The scenario starts when an eligible buyer initiates the action from `/orders/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/fulfillments/:id/status`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Fulfillment module validates the business invariant for **update fulfillment status** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. An authorized participant records a legal progress state.
...
- Emit `fulfillment.update-fulfillment-status.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/fulfillments/:id/status`.
...
`mutation updateFulfillmentStatus` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: An authorized participant records a legal progress state.
...
- Integration: `POST /api/v1/fulfillments/:id/status`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
