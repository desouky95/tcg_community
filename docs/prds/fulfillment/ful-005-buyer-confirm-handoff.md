# FUL-005: Buyer confirms handoff
...
The product needs a consistent, authorized way to **buyer confirms handoff**. Success means: The buyer records receipt idempotently. The scenario starts when an eligible buyer initiates the action from `/orders/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
3. Fulfillment module validates the business invariant for **buyer confirms handoff** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. The buyer records receipt idempotently.
...
- Emit `fulfillment.buyer-confirm-handoff.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation buyerConfirmHandoff` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The buyer records receipt idempotently.
