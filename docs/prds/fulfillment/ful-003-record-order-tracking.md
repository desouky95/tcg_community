# FUL-003: Record optional shipment tracking
...
| Surface | `/orders/:id` |
| Dependencies | [FUL-001](../fulfillment/ful-001-release-contact-details.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **record optional shipment tracking**. Success means: The seller records optional provider and reference data without a delivery guarantee. The scenario starts when an eligible buyer initiates the action from `/orders/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/fulfillments/:id/tracking`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Fulfillment module validates the business invariant for **record optional shipment tracking** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across fulfillments.
...
6. The client renders the canonical result and the next valid action. The seller records optional provider and reference data without a delivery guarantee.
...
- Emit `fulfillment.record-order-tracking.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/fulfillments/:id/tracking`.
...
`mutation recordOrderTracking` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The seller records optional provider and reference data without a delivery guarantee.
...
- Integration: `POST /api/v1/fulfillments/:id/tracking`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
