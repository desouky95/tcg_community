# FUL-007: Mark fulfillment failed
...
| Domain | Contact-led fulfillment |
| Actors | buyer, seller |
| Platforms | collector web, mobile |
| Surface | `/orders/:id` |
| Dependencies | [FUL-004](../fulfillment/ful-004-update-fulfillment-status.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **mark fulfillment failed**. Success means: A participant records failure and receives payment-support options without a delivery guarantee. The scenario starts when an eligible buyer initiates the action from `/orders/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/fulfillments/:id/fail`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Fulfillment module validates the business invariant for **mark fulfillment failed** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across fulfillments, disputes.
5. The outbox publishes FulfillmentScheduled, FulfillmentUpdated, HandoffConfirmed, FulfillmentFailed; noncritical notifications, projections, and analytics run asynchronously.
6. The client renders the canonical result and the next valid action. A participant records failure and receives payment-support options without a delivery guarantee.
...
- Emit `fulfillment.mark-fulfillment-failed.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/fulfillments/:id/fail`.
...
`mutation markFulfillmentFailed` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.

## Backend and data rules

- Owning component: Fulfillment module; other domains interact through application interfaces and outbox events, not direct cross-domain writes.
- Authoritative records: fulfillments(id, order_id, method, status, version, meetup_at, tracking_provider, tracking_reference); fulfillment_confirmations(id, fulfillment_id, actor_id, confirmation_type, created_at).
- Required indexes and constraints: unique fulfillments(order_id); fulfillments(status, updated_at); unique fulfillment_confirmations(fulfillment_id, actor_id, confirmation_type).
- Fulfillment status is not cached.
...
- Given valid prerequisites and authorization, the scenario ends with: A participant records failure and receives payment-support options without a delivery guarantee.
...
- Integration: `POST /api/v1/fulfillments/:id/fail`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
