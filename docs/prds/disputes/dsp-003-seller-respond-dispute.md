# DSP-003: Seller responds to dispute
...
| Dependencies | [DSP-001](../disputes/dsp-001-open-payment-dispute.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **seller responds to dispute**. Success means: The seller response is timestamped and visible to case participants. The scenario starts when an eligible buyer initiates the action from `dispute detail` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/disputes/:id/responses`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Dispute module validates the business invariant for **seller responds to dispute** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across dispute_events.
...
6. The client renders the canonical result and the next valid action. The seller response is timestamped and visible to case participants.
...
- Emit `disputes.seller-respond-dispute.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/disputes/:id/responses`.
...
`mutation sellerRespondDispute` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The seller response is timestamped and visible to case participants.
...
- Integration: `POST /api/v1/disputes/:id/responses`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
