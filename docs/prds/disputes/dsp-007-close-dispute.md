# DSP-007: Close dispute after provider result
...
| Surface | `dispute worker` |
| Dependencies | [DSP-005](../disputes/dsp-005-resolve-dispute-refund.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **close dispute after provider result**. Success means: The case closes only after required provider action reaches a terminal state. The scenario starts when the named system trigger or dependency event occurs and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `DisputeClosurePolicy`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Dispute module validates the business invariant for **close dispute after provider result** and rejects illegal or stale state without side effects.
...
6. The worker records the canonical result; dependent clients observe it through their owning query or event. The case closes only after required provider action reaches a terminal state.
...
- Emit `disputes.close-dispute.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `DisputeClosurePolicy`.
...
- Given valid prerequisites and authorization, the scenario ends with: The case closes only after required provider action reaches a terminal state.
...
- Integration: `DisputeClosurePolicy`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
