# DSP-006: Resolve without refund
...
| Dependencies | [DSP-004](../disputes/dsp-004-triage-dispute.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **resolve without refund**. Success means: A reasoned non-refund decision closes the case. The scenario starts when an eligible administrator initiates the action from `admin disputes` and ends when the canonical result is persisted or a typed terminal error is returned.
...
3. Dispute module validates the business invariant for **resolve without refund** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. A reasoned non-refund decision closes the case.
...
- Emit `disputes.resolve-dispute-no-refund.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`mutation resolveDisputeNoRefund` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A reasoned non-refund decision closes the case.
