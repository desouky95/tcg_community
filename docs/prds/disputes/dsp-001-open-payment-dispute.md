...
The product needs a consistent, authorized way to **open payment dispute**. Success means: An eligible participant opens one categorized case per order. The scenario starts when an eligible buyer initiates the action from `/orders/:id` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/disputes`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Dispute module validates the business invariant for **open payment dispute** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across disputes.
...
6. The client renders the canonical result and the next valid action. An eligible participant opens one categorized case per order.
...
- Emit `disputes.open-payment-dispute.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/disputes`.
...
`mutation openPaymentDispute` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: An eligible participant opens one categorized case per order.
...
- Integration: `POST /api/v1/disputes`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
