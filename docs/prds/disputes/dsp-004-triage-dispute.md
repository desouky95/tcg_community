...
The product needs a consistent, authorized way to **triage dispute**. Success means: An operator assigns category, priority, and due date. The scenario starts when an eligible administrator initiates the action from `admin disputes` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/admin/disputes/:id/triage`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Dispute module validates the business invariant for **triage dispute** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. An operator assigns category, priority, and due date.
...
- Emit `disputes.triage-dispute.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/admin/disputes/:id/triage`.
...
`mutation triageDispute` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: An operator assigns category, priority, and due date.
...
- Integration: `POST /api/v1/admin/disputes/:id/triage`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
