# DSP-008: Appeal dispute decision
...
| Domain | Disputes and refunds |
| Actors | buyer, seller |
| Platforms | collector web, mobile |
| Surface | `dispute detail` |
| Dependencies | [DSP-005](../disputes/dsp-005-resolve-dispute-refund.md), [DSP-006](../disputes/dsp-006-resolve-dispute-no-refund.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **appeal dispute decision**. Success means: One timely appeal reopens review without repeating the original action. The scenario starts when an eligible buyer initiates the action from `dispute detail` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/disputes/:id/appeal`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Dispute module validates the business invariant for **appeal dispute decision** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across disputes, dispute_events.
5. The outbox publishes DisputeOpened, EvidenceSubmitted, DisputeResolved, DisputeAppealed; noncritical notifications, projections, and analytics run asynchronously.
6. The client renders the canonical result and the next valid action. One timely appeal reopens review without repeating the original action.
...
- Emit `disputes.appeal-dispute.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/disputes/:id/appeal`.
...
`mutation appealDispute` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.

## Backend and data rules

- Owning component: Dispute module; other domains interact through application interfaces and outbox events, not direct cross-domain writes.
- Authoritative records: disputes(id, order_id, opened_by, category, status, resolution, version); dispute_evidence(id, dispute_id, submitted_by, object_key, description); dispute_events(id, dispute_id, actor_id, type, payload, created_at).
- Required indexes and constraints: disputes(order_id, status); disputes(status, updated_at); dispute_evidence(dispute_id); dispute_events(dispute_id, created_at).
- Dispute data is never cached and is accessible only to participants and authorized operators.
...
- Given valid prerequisites and authorization, the scenario ends with: One timely appeal reopens review without repeating the original action.
...
- Integration: `POST /api/v1/disputes/:id/appeal`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
