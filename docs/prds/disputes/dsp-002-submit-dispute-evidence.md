# DSP-002: Submit dispute evidence
...
The product needs a consistent, authorized way to **submit dispute evidence**. Success means: A scanned evidence item is appended before the deadline. The scenario starts when an eligible buyer initiates the action from `dispute detail` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/disputes/:id/evidence`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Dispute module validates the business invariant for **submit dispute evidence** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across dispute_evidence, object storage.
...
6. The client renders the canonical result and the next valid action. A scanned evidence item is appended before the deadline.
...
- Emit `disputes.submit-dispute-evidence.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/disputes/:id/evidence`.
...
`mutation submitDisputeEvidence` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A scanned evidence item is appended before the deadline.
...
- Integration: `POST /api/v1/disputes/:id/evidence`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
