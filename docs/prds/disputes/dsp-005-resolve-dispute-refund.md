...
| Platforms | admin web |
| Surface | `admin disputes` |
| Dependencies | [DSP-004](../disputes/dsp-004-triage-dispute.md), [PAY-010](../payments/pay-010-issue-provider-refund.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **resolve with refund**. Success means: A reasoned decision requests an eligible provider refund. The scenario starts when an eligible administrator initiates the action from `admin disputes` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/admin/disputes/:id/resolve`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Dispute module validates the business invariant for **resolve with refund** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across disputes, refunds.
...
6. The client renders the canonical result and the next valid action. A reasoned decision requests an eligible provider refund.
...
- Emit `disputes.resolve-dispute-refund.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/admin/disputes/:id/resolve`.
...
`mutation resolveDisputeRefund` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A reasoned decision requests an eligible provider refund.
...
- Integration: `POST /api/v1/admin/disputes/:id/resolve`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
