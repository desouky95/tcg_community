# ADM-011: Replay failed background job
...
**Current evidence:** Queue exists; operations contract absent.

## Problem and outcome

The product needs a consistent, authorized way to **replay failed background job**. Success means: An authorized operator safely replays an idempotent failed job. The scenario starts when an eligible administrator initiates the action from `operations console` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/admin/jobs/:id/replay`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Administration module validates the business invariant for **replay failed background job** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across queue failure store, audit_logs.
...
6. The client renders the canonical result and the next valid action. An authorized operator safely replays an idempotent failed job.
...
- Emit `administration.replay-failed-job.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/admin/jobs/:id/replay`.
...
`mutation replayFailedJob` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: An authorized operator safely replays an idempotent failed job.
...
- Integration: `POST /api/v1/admin/jobs/:id/replay`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
