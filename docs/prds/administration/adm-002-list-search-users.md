...
| Surface | `/admin/users` |
| Dependencies | [ADM-001](../administration/adm-001-view-operations-dashboard.md) |

**Current evidence:** apps/api AdminController.listUsers.

## Problem and outcome

The product needs a consistent, authorized way to **list and search users**. Success means: Administrators page through users using safe filters. The scenario starts when an eligible administrator initiates the action from `/admin/users` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/users`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Administration module validates the business invariant for **list and search users** and rejects illegal or stale state without side effects.
...
6. The client renders the canonical result and the next valid action. Administrators page through users using safe filters.
...
- Emit `administration.list-search-users.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.
...
`query listSearchUsers` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Administrators page through users using safe filters.
...
- Integration: `GET /api/v1/users`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
