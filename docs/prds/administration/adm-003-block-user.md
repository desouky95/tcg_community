...
**Current evidence:** apps/api AdminController.blockUser.

## Problem and outcome

The product needs a consistent, authorized way to **block user**. Success means: The account is blocked and active sessions are revoked. The scenario starts when an eligible administrator initiates the action from `/admin/users` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/admin/users/:id/block`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Administration module validates the business invariant for **block user** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across users, access_tokens, audit_logs.
...
6. The client renders the canonical result and the next valid action. The account is blocked and active sessions are revoked.
...
- Emit `administration.block-user.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/admin/users/:id/block`.
...
`mutation blockUser` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: The account is blocked and active sessions are revoked.
...
- Integration: `POST /api/v1/admin/users/:id/block`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
