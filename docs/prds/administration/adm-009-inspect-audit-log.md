...
The product needs a consistent, authorized way to **inspect audit log**. Success means: Immutable privileged actions can be filtered and exported. The scenario starts when an eligible administrator initiates the action from `/admin/audit-log` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/admin/audit-logs`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Administration module validates the business invariant for **inspect audit log** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from audit_logs.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Immutable privileged actions can be filtered and exported.
...
- Emit `administration.inspect-audit-log.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/admin/audit-logs`.
...
`query inspectAuditLog` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Immutable privileged actions can be filtered and exported.
...
- Integration: `GET /api/v1/admin/audit-logs`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
