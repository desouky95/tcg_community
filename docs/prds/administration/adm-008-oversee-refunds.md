...
| Dependencies | [PAY-010](../payments/pay-010-issue-provider-refund.md) |

**Current evidence:** No current implementation.

## Problem and outcome

The product needs a consistent, authorized way to **oversee refunds**. Success means: Authorized operators inspect refund progress and failures. The scenario starts when an eligible administrator initiates the action from `/admin/payments` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/admin/refunds`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Administration module validates the business invariant for **oversee refunds** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from refunds.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Authorized operators inspect refund progress and failures.
...
- Emit `administration.oversee-refunds.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/admin/refunds`.
...
`query overseeRefunds` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Authorized operators inspect refund progress and failures.
...
- Integration: `GET /api/v1/admin/refunds`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
