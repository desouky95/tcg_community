# ADM-006: Review user and content reports
...
The product needs a consistent, authorized way to **review user and content reports**. Success means: Operators triage reports without exposing reporter identity broadly. The scenario starts when an eligible administrator initiates the action from `/admin/reports` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/admin/reports`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Administration module validates the business invariant for **review user and content reports** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from listing_reports, review_reports, disputes.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Operators triage reports without exposing reporter identity broadly.
...
- Emit `administration.review-content-reports.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/admin/reports`.
...
`query reviewContentReports` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Operators triage reports without exposing reporter identity broadly.
...
- Integration: `GET /api/v1/admin/reports`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
