# ADM-010: Request administrative export
...
The product needs a consistent, authorized way to **request administrative export**. Success means: A scoped, expiring export is generated asynchronously. The scenario starts when an eligible administrator initiates the action from `admin export action` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `POST /api/v1/admin/exports`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Administration module validates the business invariant for **request administrative export** and rejects illegal or stale state without side effects.
4. PostgreSQL commits the aggregate change, audit/outbox record, and idempotency result atomically across admin_exports.
...
6. The client renders the canonical result and the next valid action. A scoped, expiring export is generated asynchronously.
...
- Emit `administration.request-admin-export.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `POST /api/v1/admin/exports`.
...
`mutation requestAdminExport` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: A scoped, expiring export is generated asynchronously.
...
- Integration: `POST /api/v1/admin/exports`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
