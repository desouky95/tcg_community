# ADM-001: View operations dashboard
...
**Current evidence:** client AdminDashboard exists with limited data.

## Problem and outcome

The product needs a consistent, authorized way to **view operations dashboard**. Success means: Authorized operators see bounded operational summaries. The scenario starts when an eligible administrator initiates the action from `/admin` and ends when the canonical result is persisted or a typed terminal error is returned.
...
2. The interface invokes `GET /api/v1/admin/dashboard`; middleware authenticates, rate-limits, and authorizes before loading domain data.
3. Administration module validates the business invariant for **view operations dashboard** and rejects illegal or stale state without side effects.
4. PostgreSQL returns an authorization-scoped snapshot from domain aggregate projections.
5. The response is serialized with stable pagination/version metadata where applicable.
6. The client renders the canonical result and the next valid action. Authorized operators see bounded operational summaries.
...
- Emit `administration.view-operations-dashboard.started`, `.succeeded`, and `.failed` analytics without personal or provider secrets.

## Interface contract

### REST

- Operation: `GET /api/v1/admin/dashboard`.
...
`query viewOperationsDashboard` exposes the same authorization and state rules for shared web/mobile clients. Commands return a typed payload containing the updated resource, `clientMutationId`, and domain errors. Administrative bulk transfers may remain REST-first.
...
- Given valid prerequisites and authorization, the scenario ends with: Authorized operators see bounded operational summaries.
...
- Integration: `GET /api/v1/admin/dashboard`, database constraints, idempotency, optimistic concurrency, audit/outbox write, and rollback on failure.
